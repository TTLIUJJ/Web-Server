...
import xmu.ackerman.http.handler.HandlerAdapter;
import xmu.ackerman.http.handler.HtmlHandler;
import xmu.ackerman.http.handler.entityHandler.error.Error404Handler;
...
public class ResponseService {
    ...
    */
    ...
    public static String getFilePath(String filename){
        ...
        return resourceMapper.getFilePath(filename);
    }
    ...
    */
    ...
    /**
     *  仿照 上面的write 写入socket
     * @param context
     */

    public static void write2(Context context){
        try{
            Response response = context.getResponse();
            String model = ResponseService.getResponseMessage(response);

            SelectionKey key = context.getSelectionKey();
            SocketChannel client = (SocketChannel) key.channel();
            ByteBuffer buffer = ByteBuffer.wrap(model.getBytes());
            client.write(buffer);
